...
class Detector private constructor() : SensorEventListener {
    ...
    companion object {
        ...
        internal const val FALLING_WAIST_SV_TOT = 1.0
    ...
    val x: DoubleArray = buffers.buffers[BUFFER_X]
    val y: DoubleArray = buffers.buffers[BUFFER_Y]
    val z: DoubleArray = buffers.buffers[BUFFER_Z]
    val xLPF: DoubleArray = buffers.buffers[BUFFER_X_LPF]
    val yLPF: DoubleArray = buffers.buffers[BUFFER_Y_LPF]
    val zLPF: DoubleArray = buffers.buffers[BUFFER_Z_LPF]
    ...
    val falling: DoubleArray = buffers.buffers[BUFFER_FALLING]
    val impact: DoubleArray = buffers.buffers[BUFFER_IMPACT]
    val lying: DoubleArray = buffers.buffers[BUFFER_LYING]
    ...
    override fun onSensorChanged(event: SensorEvent) {
        if (Sensor.TYPE_ACCELEROMETER == event.sensor.type) {
            ...
            // Vérification des valeurs des capteurs avant de les traiter
            if (!postX.isNaN() && !postY.isNaN() && !postZ.isNaN()) {
                ...
                Log.e(TAG, "Valeurs invalides des capteurs détectées (NaN).")
            }
        }
    }
